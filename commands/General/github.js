...
class Github extends Command {
	...
	async run (message, args, data) {
		...
		const res = await fetch("https://api.github.com/repos/Sitouxz/sitou-bot");
		const json = await res.json();

		const embed = new Discord.MessageEmbed()
			.setAuthor(this.client.user.tag, this.client.user.displayAvatarURL({ size: 512, dynamic: true, format: 'png' }))
			.setDescription("["+message.translate("general/github:CLICK_HERE")+"](https://github.com/Sitouxz/sitou-bot)")
